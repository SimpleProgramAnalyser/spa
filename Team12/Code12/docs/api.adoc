...
If a `proc` with the same name already exist in the Variable Table, then no change will occur in the Procedure Table.
...
*Description*: Get *rootNode* stored in PKB. If <<assignRootNode>> was never called since the start of the program or since the latest <<resetPKB>> call, a *NULL_POINTER* will be returned.
