...
*Description*: Returns the *STATEMENT_TYPE* of `stmtNum`. If
`stmtNum` is not a valid statement number in SIMPLE program,
this method returns NON_EXISTENT_STATEMENT_TYPE.
...
information related to the Next relationships in the processed SIMPLE program. This API
differs from the other relationships as the transitive relationship (Next*) is not stored.
...
[#getAllPreviousStatements]
`*STATEMENT_LIST* getAllPreviousStatements(*INTEGER* next, *STATEMENT_TYPE* previousType);`

*Description*: Returns a *STATEMENT_LIST* containing all the previous *STATEMENT* s
of `next` that are of `previousType`.
...
*Description*: Returns a list of pairs, where for each pair, the *Next* relationship
holds between them, and the previous is of `stmtTypeOfPrevious`, and the next
is of `stmtTypeOfNext`.
...
*Overview*: The Trees API describes the methods related to various
trees created in the Frontend and used by Query Processor. Namely,
Trees API provides a database to store and retrieve the Abstract
Syntax Tree of the entire program, and the Control Flow Graph
for procedures in the program.

* <<assignRootNode>>
* <<getRootNode>>
* <<storeCFG>>
* <<getCFG>>
* <<getProceduresWithCFG>>
* <<storeCFGBip>>
* <<getCFGBip>>
* <<getProceduresWithCFGBip>>
...
[#getProceduresWithCFG]
`*PROCEDURE_LIST* getProceduresWithCFG()`

*Description*: Retrieves the names of all procedures that have a *CFG*
associated with them in the Program Knowledge Base.
...
[#getProceduresWithCFGBip]
`*PROCEDURE_LIST* getProceduresWithCFGBip()`

*Description*: Retrieves the names of all procedures that have a *CFG*
with branching into procedures in the Program Knowledge Base.

=== Next BIP API

*Overview*: The Next BIP API provides a Table to insert and extract information related
to the NextBip relationships in the processed SIMPLE program. It is highly similar to
the Next API, just that the information stored is different.
...
with `nextStmtType` into the Next BIP Table.
...
*Description*: Returns `TRUE` if there is a NextBip relationship between `previous` and `next`,
else returns `FALSE`.
...
*Description*: Returns a *STATEMENT_LIST* containing the next *STATEMENT* s of `previous` that are of
`nextType`, taking into account branching into procedures.
*STATEMENT_LIST* returned is empty if there is no such relationship for `previous`.

'''

[#getAllNextBipStatementsTyped]
`*STATEMENT_LIST* getAllNextBipStatementsTyped(*STATEMENT_TYPE* stmtTypeOfPrevious,
*STATEMENT_TYPE* stmtTypeOfNext);`

*Description*: Returns a *STATEMENT_LIST* of statements such that `NextBip(previous, returned)`
holds and are of STATEMENT_TYPE `stmtType`.

'''

[#getAllPreviousBipStatements]
`*STATEMENT_LIST* getAllPreviousBipStatements(*INTEGER* next, *STATEMENT_TYPE* previousType);`

*Description*: Returns a *STATEMENT_LIST* containing all the previous *STATEMENT* s
of `next` that are of `previousType`, taking into account branching into procedures.
*STATEMENT_LIST* returned is empty if there is no such relationship for `next`.

'''

[#getAllPreviousBipStatementsTyped]
`*STATEMENT_LIST* getAllPreviousBipStatementsTyped(*STATEMENT_TYPE* stmtTypeOfPrevious,
*STATEMENT_TYPE* stmtTypeOfNext);`

*Description*: Returns a *STATEMENT_LIST* of all previous statements that are of *STATEMENT_TYPE*
`stmtTypeOfPrevious`, with a next of *STATEMENT_TYPE* `stmtTypeOfNext` when
taking into account branching into procedures.

'''

[#getAllNextBipTuples]
`*LIST<PAIR<STATEMENT,STATEMENT>>* getAllNextBipTuples(*STATEMENT_TYPE* stmtTypeOfPrevious,
*STATEMENT_TYPE* stmtTypeOfNext);`

*Description*: Returns a list of pairs, where for each pair, the *NextBip* relationship
holds between them, and the previous is of `stmtTypeOfPrevious`, and the next
is of `stmtTypeOfNext`, when branching into procedures is taken into account.
...
specified `format`. If there is a syntax or semantic error, `ui` will be called to
display the error.

If `optimise` is set to true, the Query Optimiser will be used to optimise the query
namely by reordering the clauses within the query.
