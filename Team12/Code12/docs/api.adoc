...
*Description*: Returns a *PROCEDURE_LIST* of procedures called by any *Call Statements*. An empty list
is returned if there are no *Call Statements* in the SIMPLE program.
...
[#getAllStatementsInProcedure]
`*STATEMENT_LIST* getAllStatementsInProcedure(*PROCEDURE* proc);`

*Description*: Returns a *STATEMENT_LIST* of all statements within the `proc`. If `proc` is not
found in Procedure Table, returns an empty *STATEMENT_LIST*.
...
*Description*: Returns a *PROCEDURE_LIST* containing the procedure in which `stmt` is found,
...
[#getAllNextStatements]
`*STATEMENT_LIST* getAllNextStatements(*INTEGER* previous);`

*Description*: Returns a *STATEMENT_LIST* containing the next *STATEMENT* of `previous`.
...
[#getAllPreviousStatements]
`*STATEMENT_LIST* getPreviousStatement(*INTEGER* next);`

*Description*: Returns a *STATEMENT_LIST* containing the previous *STATEMENT* of `next`.
...
[#setCFG]
`*VOID* storeCFG(*CFG* rootNode, *PROCEDURE* proc)`
